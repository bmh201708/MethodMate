    ...
    <div class="flex flex-col h-full">
      ...
      <div class="flex-1 overflow-y-auto mb-4 space-y-4" ref="chatContainer">
        ...
        <div v-for="message in chatState.messages" :key="`msg_${message.id}_${message.content?.length || 0}`" 
          ...
          <div :class="['max-w-[70%] rounded-lg p-4 relative', 
            ...
            <!-- Research proposal related buttons (top right corner) -->
            <div v-if="message.type === 'assistant' && !message.isError && message.isComplete && isResearchPlan(getOriginalContent(message))"
              ...
              <button 
                ...
                title="Display research proposal in the right panel"
              >
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div class="mt-4">
        ...
        >
          ...
          <span class="font-medium">Chat with MethodMate</span>
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
          </svg>
        </button>
...
const parseAndDisplayResearchPlan = (content) => {
  try {
    ...
    // 构建新的研究方案数据
    const planData = {
      title: `Research Proposal Generated by AI`,
      researchQuestions: 'AI-generated research proposal',
      methodology: `Research methodology generated based on user requirements (Generated: ${new Date().toLocaleString('en-US')})`,
      dataCollection: 'Data collection proposal formulated according to research design',
      hypotheses: [],
      experimentalDesign: '',
      analysisMethod: '',
      expectedResults: ''
    ...
    if (updatedFields >= 1) {
      ...
      // 显示成功提示
      setTimeout(() => {
        alert('Research proposal has been updated on the right panel! Please check the content of each module.')
      }, 500)
...
const handleViewInRightPanel = (message) => {
  ...
  if (!success) {
    ...
    alert('Failed to parse research proposal. Please check if the proposal format is correct.')
  }
}
