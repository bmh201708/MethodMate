...
const processFullText = async (text, retries = 3, aiService = 'coze') => {
  try {
    // 修改为分段回答格式，每段包含方法简介和直接引用的原文
    const basePrompt = `你是一位研究方法专家。请从以下学术论文中提炼定量研究方法，采用分段回答的格式。

**要求：**
1. 按照分段格式组织内容，每段包含定量研究方法简介和直接引用的原文
2. 重点关注定量研究方法的具体实施过程
3. 直接引用原文段落，不要进行提炼或重新组织
4. 不要添加任何问候语、交流性语言或解释性说明
5. 直接输出内容，不要包含"以下是提炼结果"等前缀`;

    const chatgptAdditionalRequirement = `
6. **分段结构**：每段必须包含两部分：方法简介 + 直接引用原文
7. **直接引用**：必须完整复制粘贴原文中的相关段落，保持原文的完整性
8. **不要修改原文**：引用的原文不能有任何改动、删减或重新表述
9. **标注清楚**：用双引号明确标注哪部分是直接引用的原文
10. **回答长度**：不少于1500字，确保充分的原文引用`;
...
**1. 研究假设与设计**
**定量研究方法简介：**
[对研究假设和总体设计的方法学解释，用自己的话说明这部分的方法学意义]

**定量研究方法原文：**
"[直接复制粘贴原文中关于研究假设的完整段落，不做任何修改]"

"[直接复制粘贴原文中关于研究设计的完整段落，不做任何修改]"

**2. 实验实施方法**  
**定量研究方法简介：**
[对实验具体实施过程的方法学解释，用自己的话说明这部分的方法学意义]

**定量研究方法原文：**
"[直接复制粘贴原文中关于实验实施的完整段落，不做任何修改]"

"[直接复制粘贴原文中关于被试和程序的完整段落，不做任何修改]"

**3. 数据收集与分析**
**定量研究方法简介：**
[对数据收集和分析方法的方法学解释，用自己的话说明这部分的方法学意义]

**定量研究方法原文：**
"[直接复制粘贴原文中关于数据收集的完整段落，不做任何修改]"

"[直接复制粘贴原文中关于统计分析的完整段落，不做任何修改]"

**4. 结果测量与评估**
**定量研究方法简介：**
[对结果测量和评估方法的方法学解释，用自己的话说明这部分的方法学意义]

**定量研究方法原文：**
"[直接复制粘贴原文中关于结果测量的完整段落，不做任何修改]"

"[直接复制粘贴原文中关于评估方法的完整段落，不做任何修改]"

**重要提醒：
1. 简介部分用你自己的话解释方法学意义
2. 原文部分必须是完整的段落复制，用双引号标注
3. 不要对原文进行任何修改、删减或重新表述
4. 每段至少包含2-3个完整的原文段落**

**论文文本：**
${text}

请严格按照上述格式，确保原文部分是完整复制的原始段落。`
      : basePrompt + `
...
const intelligentMergeResults = async (results, retries = 3) => {
  try {
    ...
    // 构建合并提示词
    const mergePrompt = `你是一位研究方法专家。我有 ${results.length} 个从同一篇学术论文的不同部分提取的研究方法片段，请将它们智能合并成一个完整、连贯的定量研究方法总结。

**要求：**
1. 严格按照分段回答格式输出最终结果
2. 合并时去除重复内容，整合相关信息
3. 保持信息的准确性和完整性，特别是原文引用
...
**1. 研究假设与设计**
**定量研究方法简介：**
[合并整理后的研究假设和总体设计的方法学解释]

**定量研究方法原文：**
"[合并整理后的研究假设和设计的直接原文引用，用双引号标注]"

**2. 实验实施方法**  
**定量研究方法简介：**
[合并整理后的实验具体实施过程的方法学解释]

**定量研究方法原文：**
"[合并整理后的实验实施方法的直接原文引用，用双引号标注]"

**3. 数据收集与分析**
**定量研究方法简介：**
[合并整理后的数据收集和分析方法的方法学解释]

**定量研究方法原文：**
"[合并整理后的数据收集与分析方法的直接原文引用，用双引号标注]"

**4. 结果测量与评估**
**定量研究方法简介：**
[合并整理后的结果测量和评估方法的方法学解释]

**定量研究方法原文：**
"[合并整理后的结果测量与评估方法的直接原文引用，用双引号标注]"
...
请智能合并这些片段，生成统一的分段格式研究方法总结。`;
    ...
    // 验证是否包含分段回答的基本结构
    const hasSegmentStructure = mergedText.includes('**1. 研究假设与设计**') || 
                               mergedText.includes('**2. 实验实施方法**') || 
                               mergedText.includes('**3. 数据收集与分析**') || 
                               mergedText.includes('**4. 结果测量与评估**') ||
                               mergedText.includes('**定量研究方法简介：**') ||
                               mergedText.includes('**定量研究方法原文：**');
...
3. 直接引用原文段落，不要进行提炼或重新组织
4. 不要添加任何问候语、交流性语言或解释性说明
5. 如果某部分在原文中没有明确内容，该部分输出"未明确说明"
6. 直接输出内容，不要包含"以下是提炼结果"等前缀`;

    const chatgptAdditionalRequirement = `
7. **分段结构**：每段必须包含两部分：方法简介 + 直接引用原文
8. **直接引用**：必须完整复制粘贴原文中的相关段落，保持原文的完整性
9. **不要修改原文**：引用的原文不能有任何改动、删减或重新表述
10. **充分引用**：每段至少包含2-3个完整的原文段落
11. **回答长度**：不少于1200字，确保充分的原文引用`;
...
请严格按照上述分段格式提炼定量研究方法。`;
    ...
    // 验证是否包含分段回答的基本结构
    const hasSegmentStructure = summaryText.includes('**1. 研究假设与设计**') || 
                               summaryText.includes('**2. 实验实施方法**') || 
                               summaryText.includes('**3. 数据收集与分析**') || 
                               summaryText.includes('**4. 结果测量与评估**') ||
                               summaryText.includes('**定量研究方法简介：**') ||
                               summaryText.includes('**定量研究方法原文：**');

    if (!hasSegmentStructure && summaryText.length > 50) {
      console.log('备用方法响应未按照分段格式，但内容有效，保留原始内容');
    }
