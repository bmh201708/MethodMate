...
**重要提醒：每个部分都必须包含2-3处直接引用原文的内容，用双引号标注。引用应保持原文的专业术语和具体数据。**

**论文文本：**
${text}

请严格按照上述格式提炼研究方法，确保大量引用原文内容。`
      : basePrompt + `
...
## 研究假设
[描述研究假设，必须引用原文]
示例：作者明确提出"[直接引用原文假设内容]"。

## 实验设计  
[详述实验设计，必须引用原文]
示例：研究采用"[引用原文设计描述]"，作者说明"[引用具体实验流程]"。

## 数据分析
[阐述分析方法，必须引用原文]
示例：数据分析使用"[引用统计方法原文]"，过程包括"[引用分析步骤]"。

## 结果呈现
[说明结果展示，必须引用原文]
示例：结果通过"[引用结果呈现方式原文]"展示，包括"[引用图表描述]"。
